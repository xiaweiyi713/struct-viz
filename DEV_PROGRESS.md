...
### Bug 修复
- ~~`tsc -b` 严格类型检查有约 30 个错误~~ —— 已修复（2026-09-25 验证：`tsc -b` 0 错误）
- ~~`pseudocodes.ts` 有 6 处重复属性名需去重~~ —— 已核查（2026-09-25：各 `pseudocodeLineMap` 内无重复键，无需改动）

### 功能增强
- ~~沙盒页模板选择器支持科目 Tab 筛选（类似首页科目页）~~ —— 已完成（2026-09-25：TopNav 模板下拉新增科目 Tab + 搜索框，支持按名称/英文名/描述过滤）
- 算法复杂度对比页面
- 暗色模式切换动画优化
